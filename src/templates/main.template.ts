...
const customStorage: multer.StorageEngine = {
  _handleFile(req: Request, file: Express.Multer.File, callback) {
    ...
    file.stream.on('end', () => {
      ...
      const metadataBase64 = Buffer.from(JSON.stringify(fileObj)).toString('base64');
...
async function bootstrap() {
  try {
    ...
    app.enableCors({
      ...
      allowedHeaders: [
        'Accept', 'Accept-Language', 'Connection', 'Content-Type', 'Origin', 'Referer',
        'Sec-Fetch-Dest', 'Sec-Fetch-Mode', 'Sec-Fetch-Site', 'User-Agent',
        'Sec-CH-UA', 'Sec-CH-UA-Mobile', 'Sec-CH-UA-Platform', 'language',
        'platform', 'app-version', 'Authorization', 'X-Fingerprint', 'X-RAY-ID',
      ],
    ...
    const upload = multer({ storage: customStorage, limits: { fileSize: 1024 * 1024 * 40 } });
    ...
    const config = new DocumentBuilder()
      ...
      .addBearerAuth({
        ...
        description: 'Enter JWT token', name: 'Authorization', in: 'header',
    ...
    console.log(\`🚀 Application is running at: \${appUrl}/api/v1/doc?apiKey=\${key}\`);
